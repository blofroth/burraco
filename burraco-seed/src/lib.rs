    ...
    fn update_choices(&mut self) {
        if self.game.state().player_turn == MANUAL_PLAYER {
            match self.game.phase() {
                GamePhase::Draw => {
                    self.draw_choices.append(&mut vec![format!("{}", DrawAction::DrawOpen), format!("{}", DrawAction::DrawPile)]);
                ...
                GamePhase::Play =>  {
                    ...
                    let actions = PlayAction::enumerate(
                        ...
                        &self.game.current_player().hand, 1);
    
                    let mut action_strs: Vec<_> = actions.into_iter().map(|(a, _d_score)| format!("{}", a)).collect();
    ...
    if model.game.state().player_turn == MANUAL_PLAYER {
        match (model.game.phase(), msg) {
            ...
            (GamePhase::Discard, Msg::Discard(idx)) => {
                ...
                model.last_move = format!("{} - Player {}", &curr_move, model.game.state().player_turn);
                model.game.discard(curr_move)
                    .expect("valid discard")
            },
            (p, m) => model.error_msg.push_str(&mut format!("Bad action ({:?}) in phase: {:?}", m, p))
        }
    } else {
        if let Msg::Advance = msg {
            match model.game.phase() {
                GamePhase::Draw => {
                    let curr_move = model.agents[model.game.state().player_turn].select_draw_action(model.game.state()); 
                    model.last_move = format!("{} - Player {}", &curr_move, model.game.state().player_turn);
                    model.game.draw(curr_move)
                        .expect("valid draw action");
                ...
                GamePhase::Play => { 
                    let curr_move = model.agents[model.game.state().player_turn].select_play_action(
                        ...
                        model.game.state());
                    model.last_move = format!("{} - Player {}", &curr_move, model.game.state().player_turn);
                    model.game.play(curr_move)
                        .expect("valid play action")
                },
                GamePhase::Discard => { 
                    let curr_move = model.agents[model.game.state().player_turn].select_discard_action(&model.game.current_player().hand, model.game.state());
                    model.last_move = format!("{} - Player {}", &curr_move, model.game.state().player_turn);
                    model.game.discard(curr_move)
                        .expect("valid discard");
                    model.curr_player_moves_allowed = 0;
                },
                GamePhase::Finished(winning_team) => (model.last_move = format!("Winner is team {}. Scores: {:?}", winning_team, &model.game.scoreboard()))
            }
        }
    }
    model.update_choices();
...
fn draw_action_buttons(actions: &Vec<String>) -> Node<Msg> {
    let buttons: Vec<_> = actions.iter().enumerate().map(|(action_i, s)| { 
        li![button![s, ev(Ev::Click, move |_| Msg::Draw(action_i)),]] 
        }).collect();
    ul![buttons]
}
...
fn play_action_buttons(actions: &Vec<String>) -> Node<Msg> {
    let buttons: Vec<_> = actions.iter().enumerate().map(|(action_i, s)| { 
        li![button![s, ev(Ev::Click, move |_| Msg::Play(action_i)),]] 
        }).collect();
    ul![buttons]
}
...
fn discard_action_buttons(actions: &Vec<String>) -> Node<Msg> {
    let buttons: Vec<_> = actions.iter().enumerate().map(|(action_i, s)| { 
        li![button![s, ev(Ev::Click, move |_| Msg::Discard(action_i)),]] 
        }).collect();
    ul![buttons]
}

fn card_css_suit_class(card: &Card) -> (String, String) {
    let text = if Suit::Jokers == card.0 { "Joker".into()} else { format!("{}", card.0)};
...
fn card(card: &Card) -> Node<Msg> {
     ...
     */
     let rank_upper = if card.1 == Rank::Joker { "-".to_string()} else { format!("{}", card.1) };
     let rank_lower = rank_upper.to_lowercase();
     let (suit_text, suit_class) = card_css_suit_class(&card);
     div![
         C!["card", IF!(card.1 != Rank::Joker => format!("rank-{}", rank_lower)), suit_class],
         span![
             C!["rank"],
             rank_upper
         ],
...
fn runs(runs: &Vec<Run>) -> Node<Msg> {
    ...
    let run_nodes: Vec<_> = runs.iter().map(|r| div![open_cards(r.cards()), format!("({:?}, {} p)", r.run_type(), r.score())]).collect();
    div![
        run_nodes
    ]
}
...
fn view(model: &Model) -> Node<Msg> {
    let other_team_runs: Vec<_> = model.game.state().teams[1..].iter().map(|t| runs(&t.played_runs)).collect();
    div![
        h1!("Burraco"),    
        ...
        div![
            C!["playingCards", "fourColours", "rotateHand", "simpleCards", "inText"],
            style!{
                St::Display => "flex",
                St::Padding => px(10)
            },
        ...
        div![
            ...
            div![
                ...
                "Pots", 
                deck(&model.game.state().pot1), deck(&model.game.state().pot2)],
            div![
                style!{
                    "flex" => "1"
                },
